...
Box::Box(Rectf rect, Point2f pos, Color4f color)
	: m_Rect{ rect }
	, m_Position{ pos }
	, m_Color{ color }
	, m_Text{ std::to_string(int(m_Rect.width)),  "BabyPlums-rv2gL.ttf", 40, Color4f{0.f,0.f,0.f,1.f} }
{
...
void Box::Draw() const
{
	...
	m_Text.Draw(Point2f(m_Rect.left, m_Rect.bottom));
}

void Box::UpdateCollisions(std::vector<std::vector<Point2f>> vertices, float elapsedSec)
{
...
void Box::MoveBox(Rectf playerRect, float elapsedSec, float playerSpeed)
{
...
bool Box::FuseBox(Rectf boxRect)
{
	if (utils::IsOverlapping(m_Rect, boxRect) && m_Rect.width == boxRect.width) {
		std::cout << "FUSED\n";
		return true;
	}
	return false;
...
void Box::BigBox()
{
	m_Rect.width *= 2;
	m_Rect.height *= 2;
	m_Text = Texture{ std::to_string(int(m_Rect.width)),  "BabyPlums-rv2gL.ttf", 40, Color4f{0.f,0.f,0.f,1.f} };
}
...
bool Box::GetIsHolding()
{
	return m_IsHolding;
}

void Box::UpdateYCollision(const Rectf& boxRect)
{
...
void Box::SetPosition(Point2f value)
{
	m_Rect.left = value.x;
	m_Rect.bottom = value.y;
}

void Box::SetRectWH(Point2f wh)
{
	m_Rect.width = wh.x;
	m_Rect.height = wh.y;
}